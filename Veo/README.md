...
- 另一组批量输出目录：`outputs/generated_veo_batch_v3fast/`
...
- `scripts/submit_jingang_batch.py`：批量提交金刚经分镜
- `scripts/wait_and_retry_batch.py`：轮询批量任务并自动下载/重提
- `scripts/submit_difu_travel_batch.py`：批量提交地府游记分镜
...
```bash
cd /mnt/e/ai_work/py/Veo
python3 scripts/submit_jingang_batch.py --config configs/veo_config.json
```
